...
using namespace std;
	...
	void board::loadBoard() {
				...
				if (boardStr[col][row] != "ee") {//redundant check
					...
					if (pieceType == "p" || "r" || "n" || "b" || "q" || "k") {
						if (pieceType == "p") {
							...
							sortPiecesForColour(pieceColour);
						}
						if (pieceType == "r") {
		...
		//pass information on all piece locations, to each piece
		for (int i=0; i < whitePieceNum; i++){
			whitePieces[i]->setNumSameColourPiece(whitePieceNum);
			whitePieces[i]->setSameColourPieces(whitePieces);
			whitePieces[i]->setNumOppositeColourPiece(blackPieceNum);
			whitePieces[i]->setOppositeColourPieces(blackPieces);
		}
		for (int i=0; i < blackPieceNum; i++) {
			blackPieces[i]->setNumSameColourPiece(blackPieceNum);
			blackPieces[i]->setSameColourPieces(blackPieces);
			blackPieces[i]->setNumOppositeColourPiece(whitePieceNum);
			blackPieces[i]->setOppositeColourPieces(whitePieces);
		}
	};
	/*OLD FORMAT
